...
# queuedFloors = [] #queuedFloors would be the initial set of buttons pressed when someone gets on the elevator when the elvator has no other locations
# currentFloor = 0
...
while True:
    ...
    if stopHere == True or stopHerebackup == True or stopHereup == True or stopHeredown == True: # 
        ...
        if stopHereup == True:
            calledUp.remove(currentFloor)
            for i in requestDict[currentFloor]:
                queuedFloors.append(i)
                requestDict[currentFloor].remove(i)
            print(currentFloor)
            print(stopHereup) # I used some of these prints to make sure everythings working and to keep trck of the floors
        if stopHeredown == True:
            calledDown.remove(currentFloor)
...
requestDict # the requestDict should be empty by the end of this
...
while True:
    ...
    if ((stopHereup == True and direction == "up") or (stopHeredown == True and direction == "down")) and weightCount < softCapacity: # the elevator won't stop to let new people on if it's at the softcap for weight
            if stopHereup == True and direction == "up":
                calledUp.remove(currentFloor)
                y: int = 0 # counter to counterbalance the for loop
                for i in range(len(requestDict[currentFloor])):
                    queuedFloors.append(requestDict[currentFloor][i])
